...
const preparedExtras = Promise.all([extras.springOnion,extras.broccoli,extras.roastedPeanuts].map((extra)=>prepareExtra(extra)));
...
const geraltsMeal = Promise.all([mealWithoutExtras,preparedExtras])
.then(([meal,prepExtras])=>addPreparedExtras(meal,prepExtras))
.then((meal)=>bag(meal));
  ...
  .then((theMeal) => {
    ...
    describe("Geralt's meal", () => {
      ...
      it('should have base: fine noodles', () => {
        expect(theMeal.base).toEqual(bases.fineNoodles);//riceNoodles??
      });
